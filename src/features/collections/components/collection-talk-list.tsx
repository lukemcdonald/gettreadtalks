...
export function CollectionTalkList({ talks }: CollectionTalkListProps) {
  return (
    <ol className="relative flex flex-col gap-3">
      {talks.length > 1 && (
        <div className="absolute top-6 bottom-6 left-4 w-px -translate-x-1/2 bg-border" />
      )}

      {talks.map((talk: TalkWithSpeaker, index) => (
        <li className="flex items-center gap-4" key={talk._id}>
          <div className="relative z-10 flex size-8 shrink-0 items-center justify-center rounded-full border-2 border-border bg-card font-semibold text-muted-foreground text-xs tabular-nums">
            {index + 1}
          </div>
