 ...
 */
export function generateTalks(count: number, speakerIds: Id<'speakers'>[]) {
  const talks: Talk[] = [];
  ...
  // biome-ignore lint/complexity: seed function
 ...
 */
function distributeTalksAcrossSpeakers(totalTalks: number, speakerCount: number) {
  ...
  while (remainingTalks > 0) {
    ...
    if (speakerIndex < featuredSpeakerCount) {
      ...
      if (distribution[speakerIndex] < 10) {
        distribution[speakerIndex] += 1;
        remainingTalks -= 1;
      }
    } else if (distribution[speakerIndex] < 5) {
      distribution[speakerIndex] += 1;
      remainingTalks -= 1;
    }
  }
