...
export default function SentryTestPage() {
  ...
  const testBasicError = () => {
    try {
      ...
      setLastError('Basic error captured');
    }
  };
  ...
  const testErrorWithContext = () => {
    ...
    try {
      ...
      setLastError('Error with context captured');
    }
  };
  ...
  return (
    <div className="container mx-auto max-w-2xl p-8">
      ...
      <p className="mb-8 text-gray-600">
        Test different error scenarios to verify Sentry integration.
      </p>

      <div className="mb-6 rounded-lg bg-yellow-50 p-4 text-yellow-800">
        <p className="font-semibold">⚠️ Development Mode Notice</p>
        <p className="mt-1 text-sm">
          Errors are logged to console but <strong>not sent to Sentry</strong> in development.
          Deploy to staging/production or temporarily modify{' '}
          <code className="rounded bg-yellow-100 px-1">sentry.client.config.ts</code> to test.
        </p>
      </div>
